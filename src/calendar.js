// src/calendar.js — ФИНАЛЬНАЯ ВЕРСИЯ: работает ВЕЗДЕ, включая Opera (23.11.2025)
...
// КЛЮЧЕВАЯ ФУНКЦИЯ — определяет, своя ли это запись
const isCurrentUserBooking = (booking) => {
  ...
  try {
    ...
    // Сравнение по телефону — точное
    if (savedPhone && booking.clientPhone === savedPhone) return true;

    // Сравнение по имени — гибкое (на случай "Анна" и "Анна И." и т.п.)
    if (savedName && booking.clientName) {
      const normalizedSaved = savedName.trim().toLowerCase();
      const normalizedBooking = booking.clientName.trim().toLowerCase();
      return normalizedBooking.includes(normalizedSaved) || normalizedSaved.includes(normalizedBooking);
    }

    return false;
  } catch (e) {
    // Если localStorage заблокирован (Opera, инкогнито и т.д.) — считаем чужой записью
    return false;
  }
};
